...
type ReaderUsecase interface {
	...
	GetRelatedArticles(articleId uint32, userId uint32, count int, page int) (*messages.RelatedArticlesResponse, error)
}
...
func (controller *ReaderController) GetRelatedArticles(ctx *gin.Context) {
	...
	userId := ctx.GetInt64(api.UserIdKey)
	articles, err := controller.ReaderUsecase.GetRelatedArticles(uint32(reqArticleId), uint32(userId), reqCount, reqPage)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, messages.SimpleResponse{Message: err.Error()})
		return
	}
